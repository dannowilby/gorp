...
type Candidate struct {
	...
	ChangeSignal chan *gorp.RoleTransition
}

func (candidate *Candidate) Init(state *gorp.State) *Candidate {

	candidate.State = state
	candidate.State.Role = "candidate"
	candidate.ChangeSignal = make(chan *gorp.RoleTransition, 1)
...
func (candidate *Candidate) AppendMessage(message gorp_rpc.AppendMessage, reply *gorp_rpc.AppendMessageReply) error {
	...
	candidate.ChangeSignal <- &gorp.RoleTransition{RoleName: "follower", State: candidate.State}
...
func (candidate *Candidate) Execute(ctx context.Context) {
	...
	if vote_tally > votes_threshold {
		candidate.ChangeSignal <- &gorp.RoleTransition{RoleName: "leader", State: candidate.State}
		return
	}

	candidate.ChangeSignal <- &gorp.RoleTransition{RoleName: "candidate", State: candidate.State}
}

func (candidate *Candidate) GetChangeSignal() chan *gorp.RoleTransition {
