...
func (follower *Follower) Init(state *gorp.State) *Follower {
	...
	follower.ChangeSignal = make(chan *gorp.RoleTransition, 1)
	...
	for {
		...
		case <-ticker.C:
			...
			if elapsed > time.Duration(follower.State.ElectionTimeout*1000000) {
				...
				follower.ChangeSignal <- &gorp.RoleTransition{RoleName: "candidate", State: follower.State}
			}
...
func (follower *Follower) GetChangeSignal() chan *gorp.RoleTransition {
